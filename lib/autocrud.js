...
function Autocrud(options) {
    ...
    // Optional before options
    var beforePut = options.beforePut;

    // Optional after options
    var afterDelete = options.afterDelete;

    // Optional passport middleware (mandatory if authentication is set)
    var passportMiddleware = options.passportMiddleware;
    ...
    if (getCreate) {
        if (getAuthentication) app.get(rootObjectPath, [passportMiddleware, getAuthentication], this.getRouteFn);
    ...
    if (getCreate) {
        if (getAuthentication) app.get(rootObjectPath + '/:id', [passportMiddleware, getAuthentication], this.getIdRouteFn);
    ...
    if (postCreate) {
        if (postAuthentication) app.post(rootObjectPath, [passportMiddleware, postAuthentication], this.postRouteFn);
        ...
        try {
            ...
            const doAction = () => {
              ...
              collection.update(createQuery(req, {
                  ...
                  if (err) return res.json(500, err);
                  if (modCount === 0) return res.sendStatus(404);
                  res.sendStatus(200);
    ...
    if (putCreate) {
        if (putAuthentication) app.put(rootObjectPath + '/:id', [passportMiddleware, putAuthentication], this.putIdRouteFn);
        ...
        try {
            ...
            }).then((document) =>
            {
              collection.remove(createQuery(req, {
                  ...
                  if (afterDelete) afterDelete(document);
              });
            });
        } catch (err) {
            res.json(400, err);
    ...
    if (deleteCreate) {
        if (deleteAuthentication) app.delete(rootObjectPath + '/:id', [passportMiddleware, deleteAuthentication], this.deleteIdRouteFn);
